...
def import_data(file, path=''):
    ...
    for name in messages.keys():
        df = df.append(pd.DataFrame(
            {'Message_Raw': messages[name], 'User': name}))
...
def remove_inactive_users(df, min_messages=10):
    ...
    to_keep = to_keep.loc[to_keep['Message_Raw']
                          >= min_messages, 'User'].values
...
def preprocess_data(df, min_messages=10):
    ...
    * Remove indices of images
    ...
    # Extract Time
    df['Date'] = df.apply(
        lambda row: row['Message_Raw'].split(' - ')[0], axis=1)

    if '/' in str(df.iloc[df.index[0]].Date):
        df['Date'] = pd.to_datetime(
            df['Date'], format='%d/%m/%y %H.%M', infer_datetime_format=True)
    else:
        if ',' in str(df.iloc[df.index[0]].Date):
            df['Date'] = pd.to_datetime(
                df['Date'], format='%d/%m/%y %H.%M', infer_datetime_format=True)
